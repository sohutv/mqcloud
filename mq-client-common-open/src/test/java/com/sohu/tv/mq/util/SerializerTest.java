...
public class SerializerTest {
    ...
    public void testSerialize() throws Exception {
        ObjectA objectA = generateTestObject();
        byte[] byteArray = new DefaultMessageSerializer().serialize(objectA);
        store(byteArray);
        Assert.assertTrue(byteArray.length > 1);
    }

    @Test
    public void testDeserialize() throws Exception {
        byte[] byteArray = read();
        ObjectA objectA2 = (ObjectA) new DefaultMessageSerializer().deserialize(byteArray);
        Assert.assertNotNull(objectA2);
        clean();
    }
    ...
    private void store(byte[] byteArray) throws Exception {
