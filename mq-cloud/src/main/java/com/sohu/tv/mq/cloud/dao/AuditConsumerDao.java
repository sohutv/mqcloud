...
public interface AuditConsumerDao {
     ...
     * @return
     */
    @Insert("insert into audit_consumer(aid,tid,consumer,consume_way,trace_enabled,permits_per_second,protocol) "
            + "values(#{ac.aid},#{ac.tid},#{ac.consumer},#{ac.consumeWay},#{ac.traceEnabled},#{ac.permitsPerSecond},#{ac.protocol})")
